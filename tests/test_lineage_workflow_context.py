...
class _Experiments:
    ...
    def run_personality_portrait_test_pack(self, model_version_id=None):
        ...
        return SimpleNamespace(ok=True, message="готово")


def _portrait(experiment_id: str, title: str, version_id: str, score: int):
...
def test_tests_viewmodel_never_displays_another_versions_portrait() -> None:
    ...
    assert "Другие сохранённые результаты намеренно не подставляются" in vm.subtitle
    assert vm.metrics[0].value == "0"
    assert "mdl_old" in vm.problematic_cases[0].note
...
def test_analysis_uses_exact_selected_and_current_portraits() -> None:
    service = _Experiments(
        [
            ...
            _portrait("evr_current", "Current portrait", "mdl_current", 5),
        ]
    )
...
def test_analysis_does_not_substitute_missing_portrait() -> None:
    ...
    assert "Нет портрета для: mdl_missing" in vm.subtitle
    assert "не подменяет" in vm.subtitle
...
def test_main_window_delivers_context_before_navigation() -> None:
    ...
    view_model = SimpleNamespace(
        set_lineage_context=lambda payload: calls.append(("context", payload))
    )
    ...
    window = SimpleNamespace(
        _workspace=SimpleNamespace(workspace=lambda key: target if key == "tests" else None),
