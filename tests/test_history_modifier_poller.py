...
def test_modifier_poller_owns_interval_and_starts_inactive() -> None:
    poller = HistoryModifierPoller(interval_ms=16)
...
def test_poll_once_is_silent_while_inactive() -> None:
    calls: list[str] = []
    poller = HistoryModifierPoller()
    poller.poll_requested.connect(lambda: calls.append("poll"))

    assert poller.poll_once() is False
    assert calls == []


def test_poll_once_emits_exactly_once_while_active() -> None:
    calls: list[str] = []
    poller = HistoryModifierPoller()
    poller.poll_requested.connect(lambda: calls.append("poll"))
    poller.set_active(True)
