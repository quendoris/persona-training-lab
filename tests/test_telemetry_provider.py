...
def test_fake_provider_returns_valid_snapshot() -> None:
    service = SystemTelemetryService(
        system_provider=_FakeSystemProvider(),
    ...
    assert snapshot.cpu_status == "normal"
    assert snapshot.gpu_status == "normal"
    assert snapshot.processes_status == "normal"
    assert snapshot.status == "active"
    assert snapshot.cpu_status_code == "normal"
...
def test_processes_unavailable_does_not_crash() -> None:
    service = SystemTelemetryService(
        system_provider=_NoProcessSystemProvider(),
        gpu_provider=None,
    )

    snapshot = service.collect_snapshot()
    assert snapshot.processes == ()
    assert snapshot.processes_status == "processes_unavailable"
...
def test_provider_error_to_viewmodel_controlled_error() -> None:
    service = SystemTelemetryService(
        system_provider=_FailingSystemProvider(),
        gpu_provider=None,
    )
...
def test_formatting_cpu_ram_vram_values() -> None:
    ...
    assert labels["Темп"].full_label == "Температура"
    assert labels["Проц"].tooltip == "Норма"


def test_nvidia_provider_missing_binary_is_soft_failure(
