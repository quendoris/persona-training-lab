...
class VersionGraphCanvas(CleanLayoutVersionGraphCanvas):
    """Size the scrollable workspace from real graph bounds and manual offsets."""
    ...
    def _content_bounds(self) -> WorkspaceBounds:
        ...
        for node_id, (x, y) in positions.items():
            ...
            label_width = self._label_width(node.title) if node is not None else 120.0
            left = min(left, x - 34.0)
            top = min(top, y - 34.0)
            right = max(right, x + 28.0 + max(64.0, label_width + 28.0))
