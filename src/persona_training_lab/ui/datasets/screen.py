...
class DatasetsScreen(QWidget):
    def __init__(self, view_model: DatasetsViewModel) -> None:
        ...
        self._subtitle = make_muted_label("JSONL, структурная проверка и readiness для обучения личности.")
        ...
        self._add_dataset_btn = QPushButton("Добавить датасет")
        actions_layout.addWidget(self._add_dataset_btn)
        self._validate_dataset_btn = QPushButton("Проверить датасет")
        actions_layout.addWidget(self._validate_dataset_btn)
        self._approve_dataset_btn = QPushButton("Одобрить для обучения")
        ...
        self._datasets_card = PanelCard("Реестр датасетов", "Curated-наборы без оценки смысла автопроверкой.")
        ...
        self._versions_card = PanelCard("Версии", "Пока одна версия на файл; сравнение включим после версий датасета.")
        ...
        self._preview_card = PanelCard("Предпросмотр записей", "Реальные строки JSONL: структура, схема и статус проверки.")
        self._preview_card.setMinimumWidth(800)
        self._table = QTableWidget(0, 4)
        self._table.setHorizontalHeaderLabels(["ID", "Вход", "Схема", "Статус"])
        ...
        self._validation_card = PanelCard("Результат проверки", "Строго проверяем только JSONL-структуру и обязательные поля.")
        self._validation_scroll, self._validation_wrap, self._validation_grid = _stable_scroll_grid(380)
        ...
        self._quality_card = PanelCard("Качество и готовность", "Автопроверка — только структура; смысл утверждает автор.")
    ...
    def _refresh_table(self) -> None:
        ...
        self._table.setColumnWidth(1, max(self._table.columnWidth(1), 280))
        self._table.setColumnWidth(2, max(self._table.columnWidth(2), 180))
    ...
    def _refresh_validation(self) -> None:
        ...
        for index, signal in enumerate(signals):
            ...
            if signal.state == "warning":
                badge = make_status_label("ошибка", warning=True)
    ...
    def _on_add_dataset(self) -> None:
        file_path, _ = QFileDialog.getOpenFileName(self, "Выберите JSONL датасет", "", "JSONL (*.jsonl)")
        if not file_path:
            return
