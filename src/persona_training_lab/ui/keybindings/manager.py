...
class KeyBindingManager(QObject):
    ...
    def set_sequence(self, binding_id: str, sequence: str) -> BindingChangeResult:
        ...
        if not definition.editable:
            return BindingChangeResult(
                ...
                error="Это назначение нельзя изменить.",
            )
        ...
        if not normalized:
            return BindingChangeResult(
                ...
                False,
                self.sequence(binding_id),
                error="Введите хотя бы одну клавишу или сочетание.",
            )
        ...
        error = self._write(updated, self._mouse_bindings)
        if error:
    ...
    def set_mouse_binding(
        ...
        previous = self._mouse_bindings.get(binding_id, MouseBindingValue("left", "none"))
        if definition is None:
            return MouseBindingChangeResult(
                False,
                False,
                previous,
                error="Неизвестный жест мыши.",
            )
        ...
        if definition.trigger == "wheel" and normalized_button != "wheel":
            return MouseBindingChangeResult(
                ...
                False,
                previous,
                error="Масштабирование должно оставаться на колесе мыши; можно изменить его модификатор.",
            )
        if definition.trigger != "wheel" and normalized_button == "wheel":
            return MouseBindingChangeResult(
                False,
                False,
                previous,
                error="Колесо нельзя назначить действию клика или перемещения.",
            )
        ...
        if conflict is not None:
            return MouseBindingChangeResult(
                ...
                False,
                previous,
                error="Этот жест уже занят другим действием в той же области.",
                conflict_binding_id=conflict.binding_id,
                conflict_title=conflict.title,
            )
        ...
        error = self._write(self._bindings, updated)
        if error:
            return MouseBindingChangeResult(False, False, previous, error=error)
    ...
    def reset_all(self) -> BindingChangeResult:
        ...
        error = self._write(defaults, mouse_defaults)
        if error:
    ...
    def _load(self) -> None:
        ...
        if not isinstance(payload, dict):
            self._last_error = "Файл назначений имеет неподдерживаемый формат."
            return
        version = payload.get("version")
        if version != self._FORMAT_VERSION and version not in self._SUPPORTED_OLD_VERSIONS:
            self._last_error = "Файл назначений имеет неподдерживаемый формат."
            return
    ...
    def _load_keyboard_bindings(self, raw_bindings: dict) -> dict[str, str]:
        ...
        if any(len(ids) > 1 for ids in groups.values()):
            self._last_error = (
                "Конфликтующие клавиатурные назначения в файле были возвращены к значениям по умолчанию."
            )
            return defaults
        return loaded

    def _load_mouse_bindings(self, raw_bindings: dict) -> dict[str, MouseBindingValue]:
        ...
        for definition in self._mouse_definitions:
            ...
            if button not in MOUSE_BUTTON_LABELS or modifier not in MOUSE_MODIFIER_LABELS:
                continue
        ...
        try:
            for definition in self._mouse_definitions:
                ...
                if self.mouse_conflict_for(definition.binding_id, candidate) is not None:
                    self._last_error = (
                        "Конфликтующие жесты мыши в файле были возвращены к значениям по умолчанию."
                    )
                    return defaults
        finally:
    ...
    def _write(
        ...
        try:
            ...
            try:
                ...
                pass
            self._last_error = f"Не удалось сохранить назначения: {exc}"
            return self._last_error
        return ""
